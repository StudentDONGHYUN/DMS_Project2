...
class DMSApp:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.critical(f"S-Class DMS 시스템 초기화 치명적 실패: {e}", exc_info=True)
            return False
                    ...
                    try:
                        ...
                        if current_time - last_performance_optimization > 10.0:
                            if frame_processing_times:
                                avg_fps = len(frame_processing_times) / sum(frame_processing_times)
                                
                                if avg_fps < min_fps and performance_mode != "emergency":
                                    performance_mode = "emergency"
                                    frame_skip_ratio = 3  # 2프레임 건너뛰기
                                    logger.warning(f"성능 최적화: 긴급 모드 ({avg_fps:.1f} FPS)")
                                elif avg_fps < target_fps * 0.8 and performance_mode == "normal":
                                    performance_mode = "optimized" 
                                    frame_skip_ratio = 2  # 1프레임 건너뛰기
                                    logger.info(f"성능 최적화: 최적화 모드 ({avg_fps:.1f} FPS)")
                                elif avg_fps > target_fps * 0.9 and performance_mode != "normal":
                                    performance_mode = "normal"
                                    frame_skip_ratio = 1  # 모든 프레임 처리
                                    logger.info(f"성능 복구: 정상 모드 ({avg_fps:.1f} FPS)")
                        ...
                        if skip_frame_counter % frame_skip_ratio != 0:
                            ...
                            if hasattr(self, '_last_annotated_frame') and self._last_annotated_frame is not None:
                                await self._async_enqueue_frame(self._last_annotated_frame)
                            continue
    ...
    async def _safe_process_frame_with_error_recovery(self, frame, frame_count):
        ...
        try:
            ...
            return frame if frame is not None else OpenCVSafeHandler.create_fallback_frame()
