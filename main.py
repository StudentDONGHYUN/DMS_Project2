...
class SClass_DMS_GUI_Setup:
    ...
    def __init__(self, root):
        ...
        self.config = {"_initialized": False, "_error": None}
        self.video_files = []
    ...
    def start_app(self):
        ...
        try:
            ...
            self.config = {"_error": f"설정 중 예외: {e}", "_initialized": True}
        finally:
...
def main():
    ...
    try:
        if GUI_AVAILABLE:
            ...
            logger.debug(
                f"GUI에서 받은 config 키들: {list(config.keys()) if isinstance(config, dict) else type(config)}"
            )
        else:
        ...
        if config and isinstance(config, dict):
            if config.get("_initialized") and not config.get("_error"):
                # 정상적으로 초기화된 config
                logger.info(
                    f"S-Class 설정 완료 - 사용자: {config.get('user_id', 'unknown')}, 에디션: {config.get('edition', 'unknown')}"
                )
                print("\n" + "=" * 70)
