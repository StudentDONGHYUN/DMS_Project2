...
class OpenCVSafeHandler:
    ...
    @staticmethod
    def safe_umat_convert(
        frame: Union[np.ndarray, cv2.UMat],
    ...
    @staticmethod
    def safe_frame_flags_writeable(
        frame: Union[np.ndarray, cv2.UMat], writeable: bool = False
    ) -> Union[np.ndarray, cv2.UMat]:
        """
    ...
    def safe_numpy_to_umat(numpy_frame):
        ...
        try:
            ...
            if isinstance(numpy_frame, np.ndarray):
                ...
                if (
                    numpy_frame.ndim == 3
                    and numpy_frame.shape[2] == 3
                    and numpy_frame.dtype == np.uint8
                ):
                    return cv2.UMat(numpy_frame)
                elif numpy_frame.ndim == 2 and numpy_frame.dtype == np.uint8:
                    return cv2.UMat(numpy_frame)
                else:
                    logger.debug(
                        f"UMat 변환 불가능한 형태: {numpy_frame.shape}, {numpy_frame.dtype}"
                    )
                    return numpy_frame
...
def safe_frame_preprocessing_for_mediapipe(frame):
    ...
    try:
        ...
        processed_frame = OpenCVSafeHandler.safe_frame_flags_writeable(
            frame, writeable=False
        )
