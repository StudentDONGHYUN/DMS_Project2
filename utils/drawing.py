...
def draw_landmarks_on_image(
    ...
    height, width = (
        annotated_image.get().shape[:2]
        if isinstance(annotated_image, cv2.UMat)
        else annotated_image.shape[:2]
    )
    # 랜드마크 점들 그리기
...
def draw_face_landmarks_on_image(rgb_image: np.ndarray, detection_result) -> np.ndarray:
    ...
    if (
        ...
        or not hasattr(detection_result, "face_landmarks")
        or not detection_result.face_landmarks
    ):
        return rgb_image
    ...
    try:
        for face_landmarks in detection_result.face_landmarks:
            ...
            if len(face_landmarks) >= 468:  # 전체 face mesh
                ...
                left_iris_center = (
                    face_landmarks[468] if len(face_landmarks) > 468 else None
                )
                right_iris_center = (
                    face_landmarks[473] if len(face_landmarks) > 473 else None
                )
                ...
                if left_iris_center:
                    left_center = (
                        int(left_iris_center.x * width),
                        int(left_iris_center.y * height),
                    )
                    cv2.circle(
                        annotated_image, left_center, 5, DrawingColors.FACE_IRISES, -1
                    )

                if right_iris_center:
                    right_center = (
                        int(right_iris_center.x * width),
                        int(right_iris_center.y * height),
                    )
                    cv2.circle(
                        annotated_image, right_center, 5, DrawingColors.FACE_IRISES, -1
                    )
...
def draw_pose_landmarks_on_image(rgb_image: np.ndarray, detection_result) -> np.ndarray:
    ...
    if (
        not detection_result
        or not hasattr(detection_result, "pose_landmarks")
        or not detection_result.pose_landmarks
    ):
        return rgb_image
...
def draw_hand_landmarks_on_image(rgb_image: np.ndarray, detection_result) -> np.ndarray:
    ...
    if (
        not detection_result
        or not hasattr(detection_result, "hand_landmarks")
    ...
    try:
        ...
        for idx, hand_landmarks in enumerate(hand_landmarks_list):
            ...
            if idx < len(handedness_list) and handedness_list[idx]:
                ...
                hand_color = (
                    DrawingColors.LEFT_HAND
                    if handedness_category == "Left"
                    else DrawingColors.RIGHT_HAND
                )
            else:
...
def draw_detection_boxes(
    ...
    try:
        for detection in detections:
            ...
            # 바운딩 박스 그리기
            cv2.rectangle(
                annotated_image, (left, top), (right, bottom), color, thickness
            )
