...
class ARHUDSystem:
    ...
    def __init__(self, screen_width: int = 1920, screen_height: int = 1080):
        ...
        print(f"   예측 하이라이팅: {'ON' if self.enable_predictive_highlighting else 'OFF'}")

    async def process_frame(self, ui_state: UIState, vehicle_context: VehicleContext) -> np.ndarray:
        """프레임 처리 및 AR 오버레이 생성"""
        ...
        # 4. AR 객체 생성/업데이트
        await self._update_ar_objects(ui_state, vehicle_context, context_info, driver_intention)
    ...
    async def _update_ar_objects(self, ui_state: UIState, vehicle_context: VehicleContext, 
                                context_info: Dict[str, Any], driver_intention: Dict[str, Any]):
        """AR 객체 생성 및 업데이트"""
    ...
    async def _create_safety_warnings(self, ui_state: UIState, context_info: Dict[str, Any]):
        """안전 경고 AR 객체 생성"""
    ...
    async def _create_lane_change_assistance(self, driver_intention: Dict[str, Any], 
    ...
    def _is_object_in_gaze_region(self, ar_obj: ARObject, gaze_region: GazeRegion) -> bool:
        """AR 객체가 시선 영역에 있는지 확인"""
        obj_x, obj_y = ar_obj.screen_position
        
        # 간단한 영역 판정 (실제로는 더 정교한 계산 필요)
        if gaze_region == GazeRegion.CENTER:
            return (self.screen_width//4 <= obj_x <= 3*self.screen_width//4 and
                    self.screen_height//4 <= obj_y <= 3*self.screen_height//4)
    ...
    async def _cleanup_expired_objects(self):
        ...
        for obj_id, ar_obj in self.ar_objects.items():
            # 페이드 지속시간이 있는 객체 체크
            if (ar_obj.fade_duration > 0 and 
                current_time - ar_obj.created_at > ar_obj.fade_duration):
    ...
    def _calculate_maneuver_position(self, direction: str, distance: float) -> Tuple[float, float, float]:
        """조작 위치 계산"""
    ...
    def _check_blind_spot_risk(self, direction: str, vehicle_context: VehicleContext) -> bool:
        """사각지대 위험 체크"""
    ...
    def get_ar_statistics(self) -> Dict[str, Any]:
        ...
        for ar_obj in self.ar_objects.values():
            ...
            stats["objects_by_priority"][priority] = stats["objects_by_priority"].get(priority, 0) + 1
...
class GazeRegionTracker:
    ...
    def determine_gaze_region(self, gaze_x: float, gaze_y: float) -> GazeRegion:
        ...
        for region, bounds in self.region_boundaries.items():
            if (bounds['x_min'] <= gaze_x <= bounds['x_max'] and
                bounds['y_min'] <= gaze_y <= bounds['y_max']):
                return region
...
class ContextAnalyzer:
    ...
    async def analyze_situation(self, vehicle_context: VehicleContext, 
                              gaze_state: DriverGazeState) -> Dict[str, Any]:
        """상황 분석"""
        analysis = {
            'risk_level': 'low',
            'high_risk_objects': [],
        ...
        for obj in vehicle_context.detected_objects:
            ...
            if risk_score > 0.7:
                analysis['high_risk_objects'].append({
                    **obj,
    ...
    def _calculate_object_risk(self, obj: Dict[str, Any], context: VehicleContext) -> float:
        """객체 위험도 계산"""
        base_risk = 0.0
...
class IntentionPredictor:
    ...
    async def predict_intention(self, gaze_state: DriverGazeState, 
                              vehicle_context: VehicleContext, 
...
class ARRenderer:
    ...
    async def render_frame(self, ar_objects: Dict[str, ARObject], 
        ...
        sorted_objects = sorted(
            ar_objects.values(),
            key=lambda obj: obj.priority.value
        )
    ...
    async def _draw_warning_box(self, x: int, y: int, w: int, h: int, 
                               color: Tuple[int, int, int, int], text: str):
        """경고 박스 그리기"""
        # 박스 테두리
        cv2.rectangle(self.frame_buffer, (x-w//2, y-h//2), (x+w//2, y+h//2), color, 3)
        ...
        if text:
            ...
            cv2.putText(self.frame_buffer, text, (text_x, text_y), 
                       font, font_scale, color[:3], thickness)
    
    async def _draw_navigation_arrow(self, x: int, y: int, w: int, h: int, 
                                    color: Tuple[int, int, int, int], icon: str):
        """네비게이션 화살표 그리기"""
        # 화살표 포인트 계산
        if "right" in icon:
            points = np.array([
    ...
    async def _draw_hazard_marker(self, x: int, y: int, w: int, h: int, 
        ...
        # 느낌표
        cv2.putText(self.frame_buffer, "!", (x-5, y+5), 
                   cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 0), 2)
    
    async def _draw_text_overlay(self, x: int, y: int, color: Tuple[int, int, int, int], text: str):
        """텍스트 오버레이 그리기"""
        if text:
            cv2.putText(self.frame_buffer, text, (x, y), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, color[:3], 1)
    ...
    def _adjust_color_for_brightness(self, color: Tuple[int, int, int, int]) -> Tuple[int, int, int, int]:
        """밝기에 따른 색상 조정"""
        # 주간/야간 모드에 따른 색상 적응
        # 실제로는 주변 조도 센서 데이터 활용
        r, g, b, a = color
