...
class SClassAdvancedUIManager:
    ...
    def _render_minimal_mode(self, frame, ui_state: UIState):
        ...
        # 최소한의 상태 표시 (우측 상단)
        status_text = (
            f"SAFE" if ui_state.risk_score < 0.3 else f"RISK: {ui_state.risk_score:.1f}"
        )
        status_color = (
            self.colors["success_green"]
            if ui_state.risk_score < 0.3
            else self.colors["warning_amber"]
        )
    ...
    def _draw_main_status_panel_uistate(self, frame, ui_state: UIState):
        ...
        # 패널 배경
        overlay = frame.copy()
        panel_points = np.array(
            [[10, 10], [panel_w, 10], [panel_w, panel_h], [10, panel_h]], np.int32
        )
        ...
        cv2.putText(
            frame,
            f"RISK SCORE: {ui_state.risk_score:.2f}",
            (20, y_current),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            risk_color,
        ...
        cv2.putText(
            frame,
            f"STATUS: {ui_state.overall_safety_status.upper()}",
            (20, y_current),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            risk_color,
        ...
        if ui_state.biometrics.heart_rate:
            ...
            cv2.putText(
                frame,
                f"♥ HR: {ui_state.biometrics.heart_rate:.0f} BPM",
                (20, y_current),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                hr_color,
        ...
        cv2.putText(
            frame,
            f"👁 ATTENTION: {ui_state.gaze.attention_score:.2f}",
            (20, y_current),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            self.colors["success_green"]
    ...
    def _draw_biometric_panel_uistate(self, frame, ui_state: UIState):
        ...
        if ui_state.biometrics.heart_rate:
            ...
            cv2.putText(
                frame,
                f"♥ {ui_state.biometrics.heart_rate:.0f} BPM",
                (panel_x + 10, y_current),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                pulse_color,
                2,
        ...
        if ui_state.biometrics.stress_level is not None:
            ...
            cv2.putText(
                frame,
                f"📊 STRESS: {ui_state.biometrics.stress_level:.2f}",
                (panel_x + 10, y_current),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                stress_color,
        ...
        cv2.putText(
            frame,
            f"😐 {ui_state.face.emotion_state.value.upper()}",
            (panel_x + 10, y_current),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            emotion_color,
    ...
    def _draw_system_health_panel(self, frame, ui_state: UIState):
        ...
        fps_color = (
            self.colors["success_green"]
            if ui_state.system_health.processing_fps > 15
            else self.colors["warning_amber"]
        )
        cv2.putText(
            frame,
            f"FPS: {ui_state.system_health.processing_fps:.1f}",
            (panel_x + 10, y_current),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            fps_color,
        ...
        status_color = self.status_colors.get(
            ui_state.system_health.overall_status.upper(), self.colors["text_white"]
        )
        cv2.putText(
            frame,
            f"STATUS: {ui_state.system_health.overall_status.upper()}",
            (panel_x + 10, y_current + 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            status_color,
            1,
    ...
    def _draw_critical_warning_center(self, frame, ui_state: UIState):
        ...
        text_size = cv2.getTextSize(
            warning_text, cv2.FONT_HERSHEY_DUPLEX, font_scale, thickness
        )[0]
        x = (w - text_size[0]) // 2
        y = h // 2
        ...
        warning_color = tuple(
            int(c * pulse_intensity) for c in self.colors["critical_magenta"]
        )
    ...
    def _draw_critical_metrics(self, frame, ui_state: UIState):
        ...
        if ui_state.biometrics.heart_rate:
            cv2.putText(
                frame,
                f"♥ {ui_state.biometrics.heart_rate:.0f}",
                (x, y),
                cv2.FONT_HERSHEY_DUPLEX,
                1.0,
                self.colors["danger_red"],
        ...
        cv2.putText(
            frame,
            f"👁 {ui_state.gaze.attention_score:.2f}",
            (x, y),
            cv2.FONT_HERSHEY_DUPLEX,
            1.0,
    ...
    def _draw_attention_warning(self, frame, ui_state: UIState):
        ...
        if ui_state.gaze.attention_score < 0.7:
            ...
            # 중앙 상단에 경고
            text_size = cv2.getTextSize(warning_text, cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)[
                0
            ]
            x = (w - text_size[0]) // 2
            y = 60
    ...
    def _draw_main_status_panel(self, frame, metrics, state):
        ...
        panel_points = np.array(
            [[0, 0], [panel_w, 0], [panel_w, panel_h], [0, panel_h]], np.int32
        )
        ...
        cv2.addWeighted(
            panel_region,
            0.7,
            frame[10 : 10 + panel_h, 10 : 10 + panel_w],
            0.3,
            0,
            frame[10 : 10 + panel_h, 10 : 10 + panel_w],
        )
        ...
        if risk_level:
            ...
            risk_color = self.risk_colors.get(
                risk_level.name, self.colors["text_white"]
            )
            cv2.putText(
                frame,
                risk_text,
            ...
            cv2.rectangle(
                frame, (bar_x, bar_y), (bar_x + fill_w, bar_y + bar_h), risk_color, -1
            )
        ...
        self._draw_metric_line(
            frame, "EAR", f"{ear_value:.3f}", (20, y_current), ear_value > 0.25
        )
        ...
        self._draw_metric_line(
            frame,
            "TEMPORAL ATT",
            f"{temporal_attention:.2f}",
            (220, y_current),
            temporal_attention > 0.6,
        )
        ...
        if gaze_zone:
            ...
            cv2.putText(
                frame,
                f"◆ GAZE: {gaze_zone.name} ({gaze_duration:.1f}s)",
                (20, y_current),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                self.colors["accent_cyan"],
        ...
        cv2.putText(
            frame,
            f"◆ HEAD POSE: Y{head_yaw:.1f}° P{head_pitch:.1f}° R{head_roll:.1f}°",
            (20, y_current),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            self.colors["text_silver"],
    ...
    def _draw_s_class_expert_systems_panel(self, frame, metrics):
        ...
        self._draw_expert_system_status(
            frame,
            "FACE PROCESSOR",
    ...
    def _draw_real_time_charts(self, frame, metrics):
        ...
        self._draw_line_chart(
            frame,
            "RISK SCORE",
            self.data_history["risk_scores"],
            (chart_x, chart_y),
            (chart_w // 2 - 5, chart_h // 2 - 5),
            self.colors["danger_red"],
        )
        ...
        self._draw_line_chart(
            frame,
            "ATTENTION",
            self.data_history["attention_scores"],
            (chart_x + chart_w // 2 + 5, chart_y),
            (chart_w // 2 - 5, chart_h // 2 - 5),
            self.colors["accent_cyan"],
        )
        ...
        self._draw_line_chart(
            frame,
            "HEART RATE",
            self.data_history["heart_rates"],
            (chart_x, chart_y + chart_h // 2 + 5),
            (chart_w - 10, chart_h // 2 - 5),
    ...
    def _draw_biometric_panel(self, frame, metrics):
        ...
        # rPPG 심박수
        if hasattr(metrics, "face_analysis") and "rppg" in getattr(
            metrics, "face_analysis", {}
        ):
        ...
        # 사카드 분석
        if hasattr(metrics, "face_analysis") and "saccade" in getattr(
            metrics, "face_analysis", {}
        ):
            ...
            cv2.putText(
                frame,
                f"👁 SACCADE VEL: {saccade_velocity:.3f}",
                (panel_x + 10, y_current),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                self.colors["accent_cyan"],
        ...
        if hasattr(metrics, "emotion_state"):
            ...
            cv2.putText(
                frame,
                f"😐 EMOTION: {metrics.emotion_state.value}",
                (panel_x + 10, y_current),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                emotion_color,
    ...
    def _draw_advanced_warnings(self, frame, metrics, state):
        ...
        warning_states = [
            state
            for state in [
                DriverState.FATIGUE_HIGH,
                DriverState.DISTRACTION_DANGER,
                DriverState.PHONE_USAGE,
                DriverState.MULTIPLE_RISK,
                DriverState.MICROSLEEP,
                DriverState.EMOTIONAL_STRESS,
            ]
            if state == state
        ]
        ...
        # 예측적 위험 경고
        if (
            hasattr(metrics, "predictive_risk_score")
            and metrics.predictive_risk_score > 0.6
        ):
    ...
    def _draw_metric_line(self, frame, label, value, pos, is_good):
        """메트릭 라인 그리기"""
        color = (
            self.colors["success_green"] if is_good else self.colors["warning_amber"]
        )
        cv2.putText(
            frame, f"{label}: {value}", pos, cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1
        )
    ...
    def _draw_expert_system_status(self, frame, title, subtitle, pos, status):
        ...
        cv2.putText(
            frame, f"● {title}", (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, status_color, 2
        )
        cv2.putText(
            frame,
            f"  {subtitle}",
    ...
    def _update_data_history(self, metrics):
        ...
        # 위험도 점수
        risk_score = getattr(metrics, "fatigue_risk_score", 0.0) + getattr(
            metrics, "distraction_risk_score", 0.0
        )
        ...
        if hasattr(metrics, "face_analysis") and "rppg" in getattr(
            ...
            heart_rate = (
                metrics.face_analysis["rppg"].get("estimated_hr_bpm", 0) / 100.0
            )  # 정규화
    ...
    def _draw_line_chart(self, frame, title, data, pos, size, color):
        ...
        # 제목
        cv2.putText(
            frame, title, (x + 5, y + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1
        )
        ...
        if data:
            ...
            if len(points) > 1:
                cv2.polylines(
                    frame, [np.array(points, dtype=np.int32)], False, color, 2
                )
    ...
    def _draw_3d_gaze_sphere(self, frame, center, radius, metrics):
        ...
        for lat in range(-90, 91, 30):
            ...
            for lon in range(-180, 181, 15):
                x, y, z = self._sphere_to_screen(
                    lat, lon, radius, yaw_rad, pitch_rad, center
                )
                if z > 0:  # 앞면만 그리기
        ...
        # 시선 방향 표시
        gaze_x, gaze_y, gaze_z = self._sphere_to_screen(
            0, 0, radius * 1.2, yaw_rad, pitch_rad, center
        )
        if gaze_z > 0:
            cv2.circle(
                frame, (int(gaze_x), int(gaze_y)), 5, self.colors["accent_cyan"], -1
            )
            cv2.circle(
                frame, (int(gaze_x), int(gaze_y)), 10, self.colors["accent_cyan"], 2
            )
    ...
    def _draw_performance_hud(self, frame, perf_stats):
        ...
        # FPS 표시
        fps_color = (
            self.colors["success_green"] if fps > 25 else self.colors["warning_amber"]
        )
        cv2.putText(
            frame,
            f"FPS: {fps:.1f}",
    ...
    def draw_enhanced_results_umat_safe(self, frame_umat, results, state=None):
        """UMat 안전 렌더링 - GPU 가속 유지"""
        try:
            # UMat 검증 및 변환
            if not isinstance(frame_umat, cv2.UMat):
                if isinstance(frame_umat, np.ndarray):
                    frame_umat = cv2.UMat(frame_umat)
                else:
                    logger.warning("지원하지 않는 프레임 타입")
                    return None

            # UMat 복사 (GPU 메모리에서 작업)
            annotated_umat = frame_umat.clone()

            # 기본 정보 표시 (UMat 지원 연산만 사용)
            self._draw_basic_info_umat(annotated_umat, results)

            return annotated_umat

        except Exception as e:
            logger.error(f"UMat 안전 렌더링 오류: {e}")
            # 폴백: numpy 변환 후 처리
            try:
                if isinstance(frame_umat, cv2.UMat):
                    frame_numpy = frame_umat.get()
                    result_numpy = self._draw_fallback_numpy(frame_numpy, results)
                    return cv2.UMat(result_numpy) if result_numpy is not None else None
                return None
            except Exception as e2:
                logger.error(f"폴백 렌더링도 실패: {e2}")
                return None

    def _draw_basic_info_umat(self, frame_umat, results):
        """UMat에서 기본 정보 그리기"""
        try:
            # 프레임 크기 정보
            h, w = frame_umat.rows, frame_umat.cols
            ...
            # 안전한 임계값 확인
            if self._safe_threshold_check(fatigue_score, 0.0):
                cv2.putText(
                    frame_umat,
            ...
            if self._safe_threshold_check(distraction_score, 0.0):
                cv2.putText(
                    frame_umat,
            ...
            # 경고 표시
            if self._safe_threshold_check(
                fatigue_score, 0.7
            ) or self._safe_threshold_check(distraction_score, 0.7):
                cv2.rectangle(frame_umat, (w - 200, 10), (w - 10, 100), (0, 0, 255), 3)
                cv2.putText(
                    frame_umat,
                    "WARNING",
                    (w - 180, 60),
            ...
            logger.warning(f"UMat 기본 정보 그리기 오류: {e}")

    def _safe_threshold_check(self, value, threshold):
        """안전한 임계값 확인 - numpy 배열 비교 오류 해결"""
        try:
            if value is None:
                return False
                ...
                if value.size == 0:
                    ...
                    return float(value.item()) > float(threshold)
                else:
                    # 배열 크기가 1보다 클 때 - 이 부분이 오류의 원인!
                    return bool(np.any(value > threshold))
            else:
                return False
        except Exception as e:
            logger.warning(f"임계값 확인 오류: {e}")
            return False

    def _draw_fallback_numpy(self, frame_numpy, results):
        """폴백 numpy 렌더링"""
        try:
            ...
            logger.error(f"폴백 numpy 렌더링 오류: {e}")
            return frame_numpy
