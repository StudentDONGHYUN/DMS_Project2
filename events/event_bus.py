...
class EventBus:
    ...
    def subscribe(self, handler: IEventHandler, event_types: Optional[List[EventType]] = None):
        """
        핸들러 등록
        ...
        if event_types is None:
            ...
            logger.info(f"핸들러 등록: {handler.get_handler_name()} - {len(event_types)}개 이벤트 타입")
    ...
    def unsubscribe(self, handler: IEventHandler):
        ...
        # 글로벌 핸들러에서 제거
        self._global_handlers = [ref for ref in self._global_handlers if ref() is not handler]
        
        # 특정 이벤트 핸들러에서 제거
        for event_type in list(self._handlers.keys()):
            self._handlers[event_type] = [ref for ref in self._handlers[event_type] if ref() is not handler]
        ...
        try:
            ...
            if queue.full():
                ...
                if event.priority in [EventPriority.LOW, EventPriority.MEDIUM]:
                    ...
                    # 높은 우선순위는 강제 처리
                    logger.critical(f"높은 우선순위 이벤트 강제 처리: {event.event_type.value}")
    ...
    async def _handle_event(self, event: Event):
        ...
        try:
            ...
            # 처리 완료 표시
            event.handled = True
            event.handlers_notified = {handler.get_handler_name() for handler in active_handlers}
            ...
            # 처리 결과 로깅
            if failure_count > 0:
                logger.warning(f"이벤트 {event.event_type.value} 처리 완료: {success_count}성공/{failure_count}실패")
            else:
    ...
    def _cleanup_dead_references(self):
        """끊어진 약한 참조들 정리"""
        # 글로벌 핸들러 정리
        self._global_handlers = [ref for ref in self._global_handlers if ref() is not None]
    ...
    def _log_statistics_report(self):
        ...
        logger.info(f"EventBus 통계 - 총 이벤트: {stats['total_events']}, "
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_processing_time_ms': self._event_statistics['average_processing_time'] * 1000,
            'queue_sizes': {priority.name: queue.qsize() for priority, queue in self._priority_queues.items()},
...
def get_event_bus() -> EventBus:
    ...
    if _global_event_bus is None:
        raise RuntimeError("이벤트 시스템이 초기화되지 않았습니다. initialize_event_system()을 먼저 호출하세요.")
    return _global_event_bus
...
class EventSystemMonitor:
    ...
    def _generate_recommendations(self, stats: Dict, indicators: Dict) -> List[str]:
        ...
        if not indicators["queue_congestion"]:
            recommendations.append("이벤트 큐 정체가 발생하고 있습니다. 핸들러 성능을 최적화하거나 큐 크기를 증가시키세요.")
        
        if not indicators["processing_speed"]:
            recommendations.append("이벤트 처리 속도가 느립니다. 핸들러 로직을 최적화하거나 병렬 처리를 증가시키세요.")
        
        if stats['total_events'] > 10000 and stats['average_processing_time_ms'] > 50:
            recommendations.append("높은 이벤트 볼륨과 처리 시간이 감지되었습니다. 시스템 리소스를 점검하세요.")
