...
class SafetyEventHandler(IEventHandler):
    ...
    def _validate_consecutive_occurrence(self, event: SafetyEvent) -> bool:
        ...
        """
        # 심각한 이벤트는 즉시 처리
        if event.priority == EventPriority.CRITICAL or event.priority == EventPriority.EMERGENCY:
            return True
    ...
    async def _execute_safety_response(self, event: SafetyEvent) -> bool:
        ...
        try:
            ...
            logger.info(f"안전 대응 완료: {event.event_type.value} (성공: {response_success})")
    ...
    async def _analyze_safety_patterns(self):
        ...
        try:
            ...
            recent_events = list(self.event_history)[-50:] if len(self.event_history) >= 50 else list(self.event_history)
            ...
            for event_type, frequency in type_frequency.items():
                if frequency > 10:  # 최근 50개 중 10개 이상
                    logger.warning(f"높은 빈도 감지: {event_type.value} ({frequency}회)")
    ...
    def get_safety_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_frequent_event": max(type_stats.items(), key=lambda x: x[1])[0] if type_stats else None,
            "average_confidence": np.mean([event.confidence for event in self.event_history]) if self.event_history else 0.0
        }
...
class AnalyticsEventHandler(IEventHandler):
    ...
    def __init__(self, analytics_engine=None):
        ...
        # 시스템 건강도 추적
        self.module_health = defaultdict(lambda: {"success_count": 0, "failure_count": 0})
        ...
        self.performance_benchmarks = {
            ...
            "max_processing_time": self.config.performance.max_processing_time_ms / 1000,  # 초 단위
            "min_confidence": 0.7,
    ...
    async def _handle_system_overload(self, event: SafetyEvent) -> bool:
        ...
        logger.warning(f"시스템 과부하: CPU {cpu_usage}%, Memory {memory_usage}%, Queue {processing_queue_size}")
    ...
    async def _handle_module_failure(self, event: SafetyEvent) -> bool:
        ...
        # 모듈별 복구 시도
        if self.analytics_engine:
            recovery_success = await self.analytics_engine.attempt_module_recovery(module_name)
    ...
    async def _handle_low_confidence(self, event: SafetyEvent) -> bool:
        ...
        recent_low_confidence = [
            record for record in list(self.quality_metrics)[-20:]  # 최근 20개
    ...
    async def _handle_data_quality_issue(self, event: SafetyEvent) -> bool:
        ...
        logger.warning(f"데이터 품질 문제: {issue_type} (영향받는 모듈: {affected_modules})")
    ...
    async def _handle_calibration_required(self, event: SafetyEvent) -> bool:
        ...
        if self.analytics_engine:
            if priority == 'high':
                # 즉시 캘리브레이션 실행
                await self.analytics_engine.start_immediate_calibration(calibration_type)
            else:
                # 대기열에 추가
    ...
    async def _analyze_real_time_performance(self):
        ...
        if avg_processing_time > target_time * 1.5:  # 기준의 150% 초과
            await self._trigger_performance_alert('processing_time_exceeded', {
                'current': avg_processing_time,
                'target': target_time,
                'severity': 'high' if avg_processing_time > target_time * 2 else 'medium'
            })
        
        # 신뢰도 저하 감지
        if avg_confidence < min_confidence:
            await self._trigger_performance_alert('confidence_below_threshold', {
    ...
    async def _analyze_degradation_patterns(self):
        ...
        # 처리 시간 추세 분석
        times = [record['processing_time'] for record in list(self.performance_history)[-100:]]
    ...
    async def _trigger_performance_alert(self, alert_type: str, data: Dict[str, Any]):
        ...
        performance_event = SafetyEvent(
            ...
            priority=EventPriority.HIGH if data.get('severity') == 'high' else EventPriority.MEDIUM,
    ...
    def get_analytics_statistics(self) -> Dict[str, Any]:
        ...
        # 기본 통계
        avg_processing_time = np.mean([r['processing_time'] for r in recent_performance])
        ...
        # 데이터 품질 점수
        recent_quality = list(self.quality_metrics)[-20:] if self.quality_metrics else []
        quality_score = np.mean([r['confidence'] for r in recent_quality]) if recent_quality else 0.0
        ...
        return {
            ...
            "system_status": "healthy" if avg_processing_time < self.performance_benchmarks['max_processing_time'] else "degraded"
        }
            ...
            for handler in _global_handlers:
                if handler.can_handle(event.event_type):
                    try:
                        ...
                        if not success:
                            logger.warning(f"핸들러 처리 실패: {handler.get_handler_name()} for {event.event_type.value}")
                    except Exception as e:
                        logger.error(f"핸들러 실행 중 오류: {handler.get_handler_name()}: {e}")
...
async def publish_safety_event(event_type: EventType, data: Dict[str, Any], source: str = "unknown", priority: EventPriority = EventPriority.MEDIUM):
    """안전 이벤트 발행 편의 함수"""
    event = SafetyEvent(
...
async def publish_analytics_event(event_type: EventType, data: Dict[str, Any], source: str = "unknown"):
    """분석 이벤트 발행 편의 함수"""
    event = SafetyEvent(
        event_type=event_type,
