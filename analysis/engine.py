...
class EnhancedAnalysisEngine:
    ...
    def on_face_result(self, result, image, timestamp_ms: int):
        ...
        logger = logging.getLogger(__name__)
        logger.info(
            f"[진단] engine.on_face_result 진입: ts={timestamp_ms}, result={type(result)}"
        )
        if timestamp_ms not in self.result_buffer:
            self.result_buffer[timestamp_ms] = {}
        self.result_buffer[timestamp_ms]["face"] = result
        logger.info(
            f"[진단] engine.result_buffer keys: {list(self.result_buffer.keys())}, frame_buffer keys: {list(self.frame_buffer.keys())}"
        )
        logger.info(
            f"[진단] engine.result_buffer[{timestamp_ms}]: {list(self.result_buffer[timestamp_ms].keys())}"
        )
    ...
    def on_pose_result(self, result, image, timestamp_ms: int):
        ...
        logger = logging.getLogger(__name__)
        logger.info(
            f"[진단] engine.on_pose_result 진입: ts={timestamp_ms}, result={type(result)}"
        )
        if timestamp_ms not in self.result_buffer:
            self.result_buffer[timestamp_ms] = {}
    ...
    def on_hand_result(self, result, image, timestamp_ms: int):
        ...
        logger = logging.getLogger(__name__)
        logger.info(
            f"[진단] engine.on_hand_result 진입: ts={timestamp_ms}, result={type(result)}"
        )
        if timestamp_ms not in self.result_buffer:
            self.result_buffer[timestamp_ms] = {}
    ...
    def on_object_result(self, result, image, timestamp_ms: int):
        ...
        logger = logging.getLogger(__name__)
        logger.info(
            f"[진단] engine.on_object_result 진입: ts={timestamp_ms}, result={type(result)}"
        )
        if timestamp_ms not in self.result_buffer:
            self.result_buffer[timestamp_ms] = {}
    ...
    def _try_queue_results(self, timestamp_ms: int):
        ...
        if (
            ...
            logger.info(
                f"[진단] engine._try_queue_results: frame={type(frame)}, results={type(results)}, keys={list(results.keys()) if results else None}"
            )
            if results:
                for k, v in results.items():
                    logger.info(
                        f"[진단] engine._try_queue_results: results[{k}]={type(v)}"
                    )
            if frame is not None and results is not None:
                ...
                try:
                    ...
                    logger.info(
                        "[진단] engine._try_queue_results: draw_enhanced_results 호출 완료"
                    )
                    try:
                        # 안전한 프레임 변환
                        if isinstance(annotated, cv2.UMat):
                            ...
                            try:
                                frame_to_show = cv2.UMat(annotated)
                            except (cv2.error, TypeError) as e:
                                logger.warning(f"UMat 변환 실패: {e}")
                                frame_to_show = annotated
                        else:
                            logger.warning(
                                f"지원하지 않는 프레임 타입: {type(annotated)}"
                            )
                            frame_to_show = annotated
                        ...
                        try:
                            if isinstance(annotated, cv2.UMat):
                                fallback_frame = annotated.get()
                            else:
                                fallback_frame = annotated
                            cv2.imshow("DMS", fallback_frame)
                        except Exception as e2:
                            logger.error(f"폴백 표시도 실패: {e2}")
                    logger.info(
                        "[진단] engine._try_queue_results: cv2.imshow 호출 완료"
                    )
    ...
    async def process_and_annotate_frame(
        ...
        try:
            face_task = asyncio.create_task(
                self._process_face_data_async(face_result, timestamp)
            )
            ...
            # 객체 처리 (손 위치 정보 필요)
            await self._process_object_data_async(
                object_result, hand_positions, timestamp
            )
    ...
    async def _process_face_data_async(self, face_result, timestamp):
        ...
        self.metrics.temporal_attention_score = drowsiness_result[
            "temporal_attention_score"
        ]
        self.metrics.personalized_threshold = drowsiness_result["threshold"]
        if face_result.face_blendshapes:
            emotion_result = self.emotion_recognizer.analyze_emotion(
                face_result.face_blendshapes[0].categories, timestamp
            )
        ...
        if face_result.facial_transformation_matrixes:
            self._analyze_enhanced_head_pose(
    ...
    async def _process_object_data_async(
        self, object_result, hand_positions, timestamp
    ):
        ...
        distraction_analysis = self.distraction_detector.analyze_detections(
            object_result, hand_positions, timestamp
        )
        self.metrics.distraction_objects = [
            obj["description"] for obj in distraction_analysis["detected_objects"]
        ]
        self.metrics.phone_detected = any(
            "휴대폰" in obj for obj in self.metrics.distraction_objects
        )
    ...
    def _perform_multimodal_fusion_analysis(self, timestamp: float):
        ...
        pose_data = {
            ...
            "head_nod_score": min(
                1.0, self.counter_analyzer.get_event_counts()["head_nods_2min"] / 5.0
            ),
        ...
        self.metrics.fatigue_risk_score = (
            self.multimodal_analyzer.fuse_drowsiness_signals(
                face_data, pose_data, emotion_data
            )
        )
        self.metrics.distraction_risk_score = (
            self.multimodal_analyzer.fuse_distraction_signals(
                face_data, hand_data, object_data, emotion_data
            )
        )
    ...
    def _analyze_enhanced_head_pose(self, transform_matrix):
        ...
        if new_gaze_zone != self.current_gaze_zone:
            self.current_gaze_zone, self.gaze_zone_start_time = (
                new_gaze_zone,
                time.time(),
            )
    ...
    def _analyze_enhanced_pose(self, world_landmarks):
        ...
        if np.linalg.norm(vec) > 0:
            self.latest_results["shoulder_yaw"] = np.degrees(
        ...
        hip_center = [
            (world_landmarks[23].x + world_landmarks[24].x) / 2,
        ...
        torso_angle = math.degrees(
            math.atan2(
                ...
                abs(shoulder_center[0] - hip_center[0]) + 0.1,
            )
        )
        self.latest_results["slouch_factor"] = max(
            0.0, min(1.0, (90 - abs(torso_angle)) / 90)
        )
    ...
    def _calculate_enhanced_gaze_deviation_score(self) -> float:
        ...
        final_score = (
            max(base_score, zone_weight * duration_factor) + instability_penalty
        )
    ...
    def _calculate_hands_on_wheel_confidence(self) -> float:
        ...
        for hand in hand_positions:
            ...
            if (
                wheel_zone["x1"] <= x <= wheel_zone["x2"]
                and wheel_zone["y1"] <= y <= wheel_zone["y2"]
            ):
                hands_on_wheel += 1
    ...
    def _determine_enhanced_overall_risk(self) -> RiskLevel:
        combined_risk = max(
            self.metrics.fatigue_risk_score, self.metrics.distraction_risk_score
        )
        predictive_weight = 0.3
        combined_risk = (
            combined_risk * (1 - predictive_weight)
            + self.metrics.predictive_risk_score * predictive_weight
        )
        if (
            self.metrics.emotion_state == EmotionState.STRESS
            and self.metrics.emotion_confidence > 0.7
        ):
    ...
    def _check_enhanced_state_transitions(self):
        ...
        if not any(
            [
                self.metrics.fatigue_risk_score > 0.5,
                self.metrics.distraction_risk_score > 0.5,
                self.metrics.predictive_risk_score > 0.5,
            ]
        ):
...
class CounterBasedAnalyzer:
    ...
    def get_event_counts(self) -> dict:
        ...
        return {
            "blinks_1min": sum(
                1 for ts in self.event_buffers["blinks"] if now - ts <= 60
            ),
            "yawns_5min": sum(
                1 for ts in self.event_buffers["yawns"] if now - ts <= 300
            ),
            "head_nods_2min": sum(
                1 for ts in self.event_buffers["head_nods"] if now - ts <= 120
            ),
            "gaze_deviations_1min": sum(
                1 for ts in self.event_buffers["gaze_deviations"] if now - ts <= 60
            ),
        }
...
class EnhancedMultiModalAnalyzer:
    ...
    def fuse_drowsiness_signals(self, face_data, pose_data, emotion_data) -> float:
        ...
        # 디버깅 로그 추가
        logger.debug(
            f"Drowsiness fusion input - Face: {face_data}, Pose: {pose_data}, Emotion: {emotion_data}"
        )

        # 1. 얼굴 데이터 처리 (조건 완화)
        face_available = face_data.get("available", True)  # 기본값을 True로 변경
        if face_available or any(
            face_data.get(key, 0) > 0.001
            for key in ["perclos", "enhanced_ear", "temporal_attention_score"]
        ):
            face_drowsiness = (
        ...
        if pose_available or pose_data.get("head_nod_score", 0) > 0.001:
            ...
            head_stability = (
                max(0.0, 1.0 - pose_complexity) * 0.3
            )  # 자세 불안정성도 피로 신호
        ...
        if (
            emotion_available or emotion_confidence > 0.1
        ):  # 임계값 대폭 낮춤 (0.5 → 0.1)
            emotion_fatigue = 0.0
        ...
        if total_weight == 0:
            logger.warning(
                "No modalities available for drowsiness fusion - using fallback"
            )
            # 기본적인 분석만으로라도 결과 제공
            fallback_score = self._calculate_fallback_drowsiness(
                face_data, pose_data, emotion_data
            )
        ...
        logger.debug(
            f"Final drowsiness score: {final_score} (total_weight: {total_weight})"
        )

        return final_score

    def fuse_distraction_signals(
        self, face_data, hand_data, object_data, emotion_data
    ) -> float:
        score, total_weight = 0.0, 0.0

        # 디버깅 로그 추가
        logger.debug(
            f"Distraction fusion input - Face: {face_data}, Hand: {hand_data}, Object: {object_data}, Emotion: {emotion_data}"
        )
        ...
        if face_available or any(
            face_data.get(key, 0) > 0.001
            for key in ["gaze_deviation_score", "attention_focus_score"]
        ):
        ...
        if hand_available or hand_data.get("hands_on_wheel_confidence") is not None:
            hands_on_wheel_confidence = hand_data.get(
                "hands_on_wheel_confidence", 0.5
            )  # 기본값 0.5
        ...
        if object_available or any(
            object_data.get(key, 0) > 0.001
            for key in ["distraction_score", "phone_usage_score"]
        ):
        ...
        if total_weight == 0:
            logger.warning(
                "No modalities available for distraction fusion - using fallback"
            )
            fallback_score = self._calculate_fallback_distraction(
                face_data, hand_data, object_data, emotion_data
            )
        ...
        final_score = score / total_weight
        logger.debug(
            f"Final distraction score: {final_score} (total_weight: {total_weight})"
        )

        return final_score

    def _calculate_fallback_drowsiness(
        self, face_data, pose_data, emotion_data
    ) -> float:
        ...
        if fallback_signals:
            ...
            logger.info(
                f"Fallback drowsiness signals found: {len(fallback_signals)}, average: {result}"
            )
    ...
    def _calculate_fallback_distraction(
        self, face_data, hand_data, object_data, emotion_data
    ) -> float:
        ...
        if fallback_signals:
            result = sum(fallback_signals) / len(fallback_signals)
            logger.info(
                f"Fallback distraction signals found: {len(fallback_signals)}, average: {result}"
            )
            return min(1.0, result)
        else:
