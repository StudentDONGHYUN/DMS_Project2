...
class PoseDataProcessor(IPoseDataProcessor):
    ...
    def __init__(self, metrics_updater: IMetricsUpdater):
        ...
        self.torso_center_history = deque(
            maxlen=150
        )  # 5 seconds of torso center tracking
    ...
    async def process_data(self, result, timestamp):
        ...
        if hasattr(result, "pose_landmarks"):
            logger.debug(
                f"[pose_processor] pose_landmarks: {getattr(result, 'pose_landmarks', None)}"
            )
        ...
        if pose_result.pose_landmarks:
            results["pose_2d"] = await self.process_pose_landmarks(
                pose_result.pose_landmarks[0], timestamp
            )

        if pose_result.pose_world_landmarks:
            results["pose_3d"] = await self.process_world_landmarks(
                pose_result.pose_world_landmarks[0], timestamp
            )

        # Advanced: Comprehensive analysis combining 2D and 3D analysis
        comprehensive_analysis = await self.perform_comprehensive_pose_analysis(
            results, timestamp
        )
    ...
    async def process_pose_landmarks(
        self, landmarks: Any, timestamp: float
    ) -> Dict[str, Any]:
    ...
    async def process_world_landmarks(
        self, world_landmarks: Any, timestamp: float
    ) -> Dict[str, Any]:
    ...
    async def perform_comprehensive_pose_analysis(
        self, results: Dict[str, Any], timestamp: float
    ) -> Dict[str, Any]:
        ...
        suitability, issues = self._evaluate_driving_posture_suitability(
            pose_2d, pose_3d
        )
        ...
        current_pose_summary = {
            ...
            "slouch_factor": pose_3d.get("slouch_detection", {}).get(
                "slouch_factor", 0
            ),
            "complexity": pose_3d.get("complexity", {}).get("overall_complexity", 0),
            "spinal_health": pose_3d.get("spinal_analysis", {}).get(
                "spine_health_score", 0.5
            ),
            "neck_health": pose_3d.get("spinal_analysis", {}).get(
        ...
        return {
            ...
            "recommendation": self._generate_posture_recommendation(
                suitability, issues, biomechanical_health
            ),
            ...
            "data_quality": {
                "pose_2d": pose_2d["available"],
                "pose_3d": pose_3d["available"],
            },
        }
    ...
    def _analyze_spinal_alignment(self, world_landmarks: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            nose = np.array(
                [
                    world_landmarks[lm.NOSE].x,
                    world_landmarks[lm.NOSE].y,
                    world_landmarks[lm.NOSE].z,
                ]
            )
            ls, rs = (
                world_landmarks[lm.LEFT_SHOULDER],
                world_landmarks[lm.RIGHT_SHOULDER],
            )
            ...
            if np.linalg.norm(neck_vector_2d) > MathConstants.VECTOR_NORM_MIN:
                fhp_angle = math.degrees(
                    math.acos(
                        np.clip(
                            np.dot(neck_vector_2d, vertical_vector_2d)
                            / np.linalg.norm(neck_vector_2d),
                            -1.0,
                            1.0,
                        )
                    )
            ...
            # Spinal angle
            if (
                np.linalg.norm(torso_vector) > MathConstants.VECTOR_NORM_MIN
                and np.linalg.norm(neck_vector) > MathConstants.VECTOR_NORM_MIN
            ):
                spine_angle = math.degrees(
                    math.acos(
                        np.clip(
                            np.dot(torso_vector, neck_vector)
                            / (
            ...
            neck_health_score = max(
                0, 1 - (max(0, fhp_angle - 15) / 30)
            )  # Score decreases if angle > 15 degrees
            spine_health_score = max(
                0, (spine_angle - 150) / 30
    ...
    def _analyze_postural_sway(
        ...
        try:
            ...
            torso_center = np.mean(
                [
                    ...
                    for p in [
                        world_landmarks[lm.LEFT_SHOULDER],
                        world_landmarks[lm.RIGHT_SHOULDER],
                        world_landmarks[lm.LEFT_HIP],
                        world_landmarks[lm.RIGHT_HIP],
                    ]
                ],
                axis=0,
            )
            ...
            sway_area = (
                math.pi
                * 5.991
                * np.sqrt(np.prod(np.abs(eigenvalues)))
                * (scale_factor**2)
            )
            ...
            if len(self.torso_center_history) > 1:
                ...
                for i in range(1, len(self.torso_center_history)):
                    dt = (
                        self.torso_center_history[i]["timestamp"]
                        - self.torso_center_history[i - 1]["timestamp"]
                    )
                    if dt > 0:
                        dp = np.linalg.norm(
                            self.torso_center_history[i]["position"]
                            - self.torso_center_history[i - 1]["position"]
                        )
    ...
    def _classify_sway_pattern(
        self, x_positions: np.ndarray, z_positions: np.ndarray
    ) -> str:
    ...
    def _detect_slouching_posture(self, world_landmarks: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if np.linalg.norm(torso_vector) > MathConstants.VECTOR_NORM_MIN:
                dot_product = np.dot(torso_vector, vertical_vector) / np.linalg.norm(
                    torso_vector
                )
            ...
            slouch_factor = max(
                0.0, min(1.0, (slouch_angle - 20) / 70.0)
            )  # Consider slouch starting from 20 degrees
    ...
    def _evaluate_driving_posture_suitability(
        self, pose_2d: Dict, pose_3d: Dict
    ...
    def _generate_posture_recommendation(
        ...
        if biomech_health["risk_factors"]:
            ...
            primary_recommendation = (
                biomech_health["recommendations"][0]
                if biomech_health["recommendations"]
                else "Correct your posture"
            )
            return f"{primary_risk} detected: {primary_recommendation}"

        if issues:
            return (
                f"Need improvement in: {', '.join(issues[:2])}"  # Show maximum 2 issues
            )
    ...
    def _analyze_posture_trends(self) -> Dict[str, Any]:
        ...
        if slope < -0.01:
            trend = "deteriorating"  # > 1% decline per hour
        elif slope > 0.01:
            trend = "improving"
        else:
            trend = "stable"
        ...
        health_scores = (
            np.array(
                [
                    h.get("spinal_health", 0.5) + h.get("neck_health", 0.5)
    ...
    def _predict_posture_future(
        self, suitability_slope: float, health_slope: float
    ) -> str:
        """Predict future posture changes"""
        if suitability_slope < -0.02 or health_slope < -0.01:
            return (
                "Continuous posture deterioration expected - immediate rest recommended"
            )
    ...
    def _calculate_pose_complexity(self, world_landmarks: Any) -> Dict[str, Any]:
        ...
        torso_points = [
            world_landmarks[i] for i in [11, 12, 23, 24]
    ...
    def _extract_fatigue_indicators_from_pose(self, pose_3d: Dict) -> Dict[str, float]:
        ...
        if not pose_3d.get("available"):
            return {"slouching": 0.0, "instability": 0.0}
    ...
    def _extract_distraction_indicators_from_pose(
        self, pose_3d: Dict
    ) -> Dict[str, float]:
        """Extract distraction indicators from posture"""
        if not pose_3d.get("available"):
            return {"unusual_positioning": 0.0}
        return {
            "unusual_positioning": pose_3d.get("complexity", {}).get(
    ...
    def _update_pose_metrics(self, pose_analysis: Dict[str, Any]):
        ...
        metrics_data = {
            "pose_complexity_score": pose_analysis.get(
                "distraction_indicators", {}
            ).get("unusual_positioning", 0.0),
            "slouch_factor": pose_analysis.get("fatigue_indicators", {}).get(
                "slouching", 0.0
            ),
            "posture_suitability": pose_analysis.get("driving_suitability", 0.5),
            "spinal_health_score": pose_analysis.get("biomechanical_health", {}).get(
                "spinal_health", 0.5
            ),
            "postural_stability": pose_analysis.get("biomechanical_health", {}).get(
                "stability", 1.0
            ),
        }
        ...
        try:
            ...
            update_method = getattr(
                self.metrics_updater, "update_pose_metrics", None
            ) or getattr(self.metrics_updater, "update_metrics", None)
            if update_method:
    ...
    async def _handle_no_pose_detected(self) -> Dict[str, Any]:
        ...
        # 점진적 위험도 증가 (0.0 -> 1.0)
        risk_score = min(
            self.consecutive_failures / self.max_failures_for_max_risk, 1.0
        )

        return {
            "pose_detected": False,
            "pose_analysis": {
                "driving_suitability": 1.0
                - risk_score,  # 위험도가 높을수록 운전 적합도 낮음
                "biomechanical_health": {
                    "overall_score": 1.0 - risk_score,
                    "risk_factors": ["Pose detection failed"],
                    "recommendations": ["Adjust camera position"],
                },
            },
        }
