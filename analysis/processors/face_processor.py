...
class FaceDataProcessor(IFaceDataProcessor):
    ...
    async def process_face_landmarks(
        ...
        try:
            ...
            drowsiness_data = await self.process_drowsiness_analysis(
                landmarks, timestamp
            )
    ...
    async def process_face_blendshapes(
        self, blendshapes: Any, timestamp: float
    ) -> Dict[str, Any]:
        """
    ...
    async def process_facial_transformation(
        self, transformation: Any, timestamp: float
    ) -> Dict[str, Any]:
        """
    ...
    async def process_data(
        self, data: Any, image: np.ndarray, timestamp: float
    ) -> Dict[str, Any]:
        """
        ...
        if hasattr(data, "face_landmarks"):
            logger.debug(
                f"[face_processor] face_landmarks: {getattr(data, 'face_landmarks', None)}"
            )
        ...
        if data.face_blendshapes:
            tasks.append(
                self.process_emotion_analysis(data.face_blendshapes[0], timestamp)
            )
        if data.facial_transformation_matrixes:
            tasks.append(
                self.process_head_pose_and_gaze(
                    data.facial_transformation_matrixes[0], timestamp
                )
            )
    ...
    async def process_drowsiness_analysis(
        self, landmarks: Any, timestamp: float
    ) -> Dict[str, Any]:
    ...
    async def process_head_pose_and_gaze(
        self, transformation_matrix: Any, timestamp: float
    ) -> Dict[str, Any]:
        """Advanced head pose and gaze analysis"""
        try:
            raw_head_pose = self._extract_euler_angles_from_matrix(
                transformation_matrix
            )
            stable_head_pose = self._stabilize_head_pose(raw_head_pose)
            new_gaze_zone = self.gaze_classifier.classify(
                stable_head_pose["yaw"], stable_head_pose["pitch"], timestamp
            )
    ...
    def _analyze_saccadic_movement(
        self, landmarks: Any, timestamp: float
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(self.left_eye_history) < self.config.face.saccade_min_samples:
                return {
                    "saccade_velocity_norm": 0.0,
                    "saccade_count_per_s": 0.0,
                    "gaze_fixation_stability": 1.0,
                }

            left_analysis = self._calculate_eye_movement_metrics(self.left_eye_history)
            right_analysis = self._calculate_eye_movement_metrics(
                self.right_eye_history
            )

            return {
                "saccade_velocity_norm": (
                    left_analysis["velocity"] + right_analysis["velocity"]
                )
                / 2,
                "saccade_count_per_s": (
                    left_analysis["count"] + right_analysis["count"]
                )
                / 2,
                "gaze_fixation_stability": (
                    left_analysis["stability"] + right_analysis["stability"]
                )
                / 2,
            }
    ...
    def _calculate_eye_movement_metrics(self, history: deque) -> Dict[str, float]:
        ...
        peaks, properties = find_peaks(
            velocities, height=saccade_velocity_threshold, distance=3
        )
        ...
        avg_saccade_velocity = (
            np.mean(properties["peak_heights"]) if len(peaks) > 0 else 0.0
        )
        ...
        if len(fixation_points) > 2:
            ...
            stability_score = max(
                0.0, 1.0 - dispersion / self.config.face.fixation_dispersion_max
            )
        else:
            stability_score = 1.0

        return {
            "velocity": avg_saccade_velocity,
            "count": saccade_count_per_s,
    ...
    def _analyze_pupil_dynamics(self, blendshapes: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            pupil_size_y = blendshapes_dict.get(
                "eyeLookUpLeft", 0
            ) - blendshapes_dict.get("eyeLookDownLeft", 0)
            pupil_size_x = blendshapes_dict.get(
                "eyeLookOutLeft", 0
            ) - blendshapes_dict.get("eyeLookInLeft", 0)
            pupil_diameter_est = np.linalg.norm([pupil_size_x, pupil_size_y])
            self.pupil_size_history.append(pupil_diameter_est)

            pupil_variability = (
                np.std(self.pupil_size_history)
                if len(self.pupil_size_history) > 1
                else 0.0
            )
            cognitive_load = (
                blendshapes_dict.get("browDownLeft", 0)
                + blendshapes_dict.get("browDownRight", 0)
            ) / 2
    ...
    def _estimate_heart_rate_from_rppg(self) -> Dict[str, Any]:
        ...
        try:
            ...
            noise_power = (
                np.mean(np.delete(fft_power, peak_freq_index))
                if len(fft_power) > 1
                else 1
            )
            ...
            if snr < self.config.rppg.snr_threshold:
                ...
                logger.warning(
                    f"rPPG SNR below threshold({snr:.2f} < {self.config.rppg.snr_threshold}). Skipping HRV calculation."
                )
            else:
                # Stage 3-2: Reliable peak detection (using prominence)
                prominence_threshold = np.std(filtered_signal) * 0.4
                ibi_peaks, _ = find_peaks(
                    filtered_signal,
                    height=0,
                    distance=fs * 0.5,
                ...
                if len(ibi_peaks) > 3:  # Need at least 3 IBIs for meaningful HRV
                    ...
                    if len(ibi_ms_valid) > 2:
                        ...
                        if hrv_std < self.config.rppg.hrv_std_threshold:
                            ...
                            logger.warning(
                                f"HRV standard deviation too high({hrv_std:.2f}ms). Considered noise and discarded."
                            )
    ...
    def _extract_euler_angles_from_matrix(
        self, transform_matrix: Any
    ) -> Dict[str, float]:
    ...
    def _stabilize_head_pose(self, raw_pose: Dict[str, float]) -> Dict[str, float]:
        ...
        if not self.head_pose_history:
            ...
            stable_pose = {
                "yaw": self.ema_alpha * raw_pose["yaw"]
                + (1 - self.ema_alpha) * last_pose["yaw"],
                "pitch": self.ema_alpha * raw_pose["pitch"]
                + (1 - self.ema_alpha) * last_pose["pitch"],
                "roll": self.ema_alpha * raw_pose["roll"]
                + (1 - self.ema_alpha) * last_pose["roll"],
            }
    ...
    def _calculate_gaze_deviation_score(
        ...
        yaw_score = min(
            1.0, abs(head_pose["yaw"]) / AnalysisConstants.Thresholds.HEAD_YAW_EXTREME
        )
        pitch_score = min(
            1.0, abs(head_pose["pitch"]) / AnalysisConstants.Thresholds.HEAD_PITCH_LIMIT
        )
        ...
        final_score = (
            (base_angle_score * 0.4)
            + (zone_risk * duration_factor * 0.4)
    ...
    def _update_all_metrics(self, results: Dict[str, Any]):
        ...
        if "drowsiness" in results:
            self.metrics_updater.update_drowsiness_metrics(results["drowsiness"])
        if "emotion" in results:
            self.metrics_updater.update_emotion_metrics(results["emotion"])
        if "gaze" in results:
            self.metrics_updater.update_gaze_metrics(results["gaze"])
        # S-Class specific metrics updates handled safely
        if (
            hasattr(self.metrics_updater, "update_saccade_metrics")
            and "saccade" in results
        ):
    ...
    async def _handle_no_face_detected(self) -> Dict[str, Any]:
        ...
        logger.warning(
            "No face detected - setting all facial metrics to default values"
        )
