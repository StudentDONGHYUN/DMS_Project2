...
class HandDataProcessor(IHandDataProcessor):
    ...
    async def process_data(self, result, timestamp):
        ...
        if hasattr(result, "hand_landmarks"):
            logger.debug(
                f"[hand_processor] hand_landmarks: {getattr(result, 'hand_landmarks', None)}"
            )
        if (
            not result
        ...
        comprehensive_analysis = await self._perform_comprehensive_hand_analysis(
            processed_hands, timestamp
        )
        ...
        results = {
            "hand_positions": processed_hands,
    ...
    async def process_hand_landmarks(
        self, hand_results: Any, timestamp: float
    ) -> List[Dict[str, Any]]:
        """
            ...
            self.gesture_sequence_buffer.append(
                {
                    ...
                    "handedness": handedness,
                    "gesture": gesture,
                    "zone": zone_analysis["primary_zone"],
                }
            )

        return processed_hands

    async def _perform_comprehensive_hand_analysis(
        self, hands: List[Dict], timestamp: float
    ) -> Dict[str, Any]:
        ...
        driving_technique = self._evaluate_driving_technique(
            steering_skill, distraction
        )
        ...
        return {
            ...
            "overall_hand_safety": self._calculate_overall_safety_score(
                steering_skill, distraction
            ),
        }

    def _analyze_hand_kinematics(
        self, landmarks: Any, handedness: str, timestamp: float
    ) -> Dict[str, Any]:
    ...
    def _analyze_tremor_frequency(self, handedness: str) -> Dict[str, Any]:
        ...
        try:
            ...
            dominant_freq = (
                xf[np.argmax(power_spectrum)] if len(power_spectrum) > 0 else 0.0
            )
            ...
            fatigue_power = (
                np.sum(power_spectrum[fatigue_band]) / total_power
                if total_power > MathConstants.EPSILON
                else 0.0
            )

            severity = "none"
            if fatigue_power > 0.4:
                severity = "severe"
            elif fatigue_power > 0.2:
                severity = "moderate"
            elif fatigue_power > 0.1:
                severity = "mild"

            return {
                "dominant_frequency_hz": dominant_freq,
                "fatigue_tremor_power": fatigue_power,
    ...
    def _calculate_finger_curl_angles(self, landmarks: Any) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                name: 90.0 for name in ["thumb", "index", "middle", "ring", "pinky"]
            }
    ...
    def _analyze_steering_skill(self, hands: List[Dict]) -> Dict[str, Any]:
        ...
        hands_on_wheel = [
            h for h in hands if h["zone_analysis"]["primary_zone"] == "STEERING_WHEEL"
        ]
        if not hands_on_wheel:
            return {
                "skill_score": 0.0,
                "feedback": "Hands not on steering wheel",
                "components": {},
            }

        clock_positions = [
            self._get_hand_clock_position(h["landmarks"]) for h in hands_on_wheel
        ]
        position_score = self._evaluate_clock_positions(clock_positions)
        smoothness_score = np.mean(
            [h["kinematics"]["smoothness_score"] for h in hands_on_wheel]
        )
        stability_score = np.mean(
            [1.0 - h["tremor_analysis"]["fatigue_tremor_power"] for h in hands_on_wheel]
        )
        grip_quality_score = np.mean(
            [h["grip_analysis"]["grip_quality"] for h in hands_on_wheel]
        )

        skill_score = (
            position_score * 0.4 + (smoothness_score + stability_score) / 2 * 0.3
        ) * grip_quality_score + (smoothness_score + stability_score) / 2 * 0.3
        feedback = self._generate_steering_feedback(
            skill_score, clock_positions, len(hands_on_wheel)
        )
        ...
        return {
            ...
            "components": {
                ...
                "grip_quality": grip_quality_score,
            },
        }
    ...
    def _detect_distraction_behaviors(self, hands: List[Dict]) -> Dict[str, Any]:
        ...
        phone_risk_info = self.config.distraction.object_risk_levels.get(
            "cell phone", {"risk_level": 0.9}
        )

        for hand in hands:
            if (
                hand["grip_analysis"]["grip_type"] == "precision_grip"
                and hand["zone_analysis"]["primary_zone"] != "STEERING_WHEEL"
            ):
                risk_score = max(risk_score, phone_risk_info["risk_level"])
                behaviors.append(
                    f"{hand['handedness']} hand, suspected object manipulation with precision grip"
                )
            ...
            if zone_risk > AnalysisConstants.Thresholds.RISK_HIGH:
                ...
                behaviors.append(
                    f"{hand['handedness']} hand, located in risk zone ({hand['zone_analysis']['primary_zone']})"
                )
    ...
    def _evaluate_driving_technique(
        self, steering_skill: Dict, distraction: Dict
    ) -> Dict[str, Any]:
    ...
    def _analyze_gesture_patterns(self) -> Dict[str, Any]:
        ...
        if gesture_counts:
            dominant_gesture = max(
                gesture_counts.keys(), key=lambda k: gesture_counts[k]
            )

        return {
            "pattern_detected": True,
            "dominant_pattern": dominant_gesture,
        ...
        for zone_name, bounds in vars(self.vehicle_zones).items():
            if bounds["x1"] <= x <= bounds["x2"] and bounds["y1"] <= y <= bounds["y2"]:
                return {
                    ...
                    "risk_level": self._get_zone_risk(zone_name),
                }
    ...
    def _generate_steering_feedback(
        self, score: float, positions: List, hands_count: int
    ) -> str:
        """Generate steering skill feedback"""
        if hands_count == 0:
            return "Please grip the steering wheel."
        if hands_count == 1:
            return "Use both hands on the steering wheel for safety."
        if not any(p is not None for p in positions):
            return "Please grip the steering wheel properly."
        if score > 0.8:
            return "Very stable steering technique."
    ...
    def _infer_hand_gesture(self, kinematics: Dict, zone: Dict, grip: Dict) -> str:
        ...
        if target_zone == "STEERING_WHEEL":
            if grip["grip_type"] == "power_grip":
                if velocity < 0.02:
                    return "holding_wheel_steady"
                return "steering"
            else:
                return "touching_wheel"
        if target_zone == "GEAR_LEVER" and velocity > 0.05:
            return "shifting_gear"
        if target_zone == "CENTER_CONSOLE" and velocity > 0.05:
            return "operating_console"
        if velocity > 0.25:
            return "rapid_movement"
        return "resting_hand"

    def _calculate_overall_safety_score(
        self, steering: Dict, distraction: Dict
    ) -> float:
        ...
        try:
            ...
            if analysis["hands_detected_count"] > 0:
                left_hand = next(
                    (
                        h
                        for h in analysis["hand_positions"]
                ...
                if left_hand:
                    left_hand_safe = (
                        left_hand["zone_analysis"]["primary_zone"] == "STEERING_WHEEL"
                    )
                if right_hand:
                    right_hand_safe = (
                        right_hand["zone_analysis"]["primary_zone"] == "STEERING_WHEEL"
                    )

            # Steering stability score (movement + tremor)
            steering_comps = analysis.get("steering_skill", {}).get("components", {})
            stability = (
                steering_comps.get("movement_smoothness", 0.0)
                + steering_comps.get("hand_stability", 0.0)
            ) / 2

            # Compose data according to IMetricsUpdater interface methods
            distraction_data = {
                "distraction_risk_score": analysis.get("distraction_behaviors", {}).get(
                    "risk_score", 0.0
                ),
                "left_hand_in_safe_zone": left_hand_safe,
                "right_hand_in_safe_zone": right_hand_safe,
                "hand_stability_score": stability,
                "phone_detected": analysis.get("distraction_behaviors", {}).get(
            ...
            try:
                update_method = getattr(
                    self.metrics_updater, "update_distraction_metrics", None
