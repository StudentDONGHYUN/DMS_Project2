...
class IntegratedDMSSystem:
    ...
    def __init__(
        ...
        logger.info(f"IntegratedDMSSystem 초기화 완료 - 타입: {system_type.value}, 레거시: {use_legacy_engine}")
    
    def _initialize_legacy_system(self, custom_config: Optional[Dict[str, Any]]) -> None:
        """
    ...
    def _initialize_modern_system(self, custom_config: Optional[Dict[str, Any]]) -> None:
        """
    ...
    async def initialize(self):
        ...
        # 분석 이벤트 핸들러  
        self.analytics_handler = AnalyticsEventHandler(analytics_engine=self.metrics_manager)
    ...
    def _validate_and_preprocess_input(self, frame_data: Dict[str, Any]) -> Dict[str, Any]:
        """입력 데이터 검증 및 전처리"""
    ...
    def _extract_driver_state(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        driver_state = {
            ...
            'system_performance': 'degraded' if execution_quality.get('degraded_performance') else 'normal',
    ...
    async def _publish_relevant_events(
        ...
        if fatigue_level > 0.8:
            await publish_safety_event(
                ...
                {'fatigue_level': fatigue_level, 'confidence': driver_state['analysis_confidence']},
    ...
    def _update_performance_metrics(self, processing_time: float):
        ...
        if current_time - self.last_performance_report > 30.0:  # 30초마다
            ...
            logger.info(f"성능 리포트 - 평균 처리시간: {avg_time*1000:.1f}ms, FPS: {fps:.1f}")
    ...
    async def shutdown(self):
        ...
        if self.frame_count > 0:
            avg_time = self.total_processing_time / self.frame_count
            logger.info(f"최종 성능 리포트 - 총 {self.frame_count}프레임, 평균 {avg_time*1000:.1f}ms")
